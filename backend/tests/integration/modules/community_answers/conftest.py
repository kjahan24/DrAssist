"""Shared fixtures for Community Answers module repository (integration)
tests.
...
uniquely-suffixed organization/user/community/question so tests can run
repeatedly against a shared, non-torn-down database without colliding;
there is deliberately no per-test transaction-rollback wrapper, kept
simple to match this project's established verification pattern — see
`tests.integration.modules.community_questions.conftest`'s own docstring
