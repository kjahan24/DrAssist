"""Shared setup helpers for SOAP Notes module repository tests — every
...
level deeper than every prior sub-module's helpers, since `soap_notes`
extends `clinical_notes` rather than `patient_visits` directly) to
satisfy `soap_notes`' foreign keys. Kept local to this test package
rather than in `app/`, matching the identical
`persist_organization`/`persist_patient`/`persist_doctor`/`persist_visit`
sequence `tests.integration.modules.clinical_notes._helpers` already
established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"soap-notes-test-{_unique_suffix()}@example.com"),
