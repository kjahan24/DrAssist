"""Shared setup helpers for Clinical Reasoning module repository tests —
every test needs a real, persisted `organizations` row, a `patients` row,
a `doctors` row, a `patient_visits` row, *and* a `clinical_notes` row (one
level deeper than every visit sub-module's own helpers, since
`clinical_reasoning` extends `clinical_notes` rather than
`patient_visits` directly) to satisfy `clinical_reasoning`'s foreign
keys. Kept local to this test package rather than in `app/`, matching the
...
async def persist_organization(db_session: AsyncSession) -> Organization:
    ...
    organization = Organization.create(
        ...
        name="Clinical Reasoning Test Org",
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"clinical-reasoning-test-{_unique_suffix()}@example.com"),
