"""Integration tests for `SqlAlchemyTopicFollowerRepository` against a
...
and the `(user_id, topic_id)` uniqueness constraint backing "Prevent
...
class TestTopicFollowerRoundTrip:
    async def test_save_and_reload(self, db_session: AsyncSession) -> None:
        ...
        repo = SqlAlchemyTopicFollowerRepository(db_session)

        follower = TopicFollower.create(
            user_id=user.id, organization_id=organization.id, topic_id=topic.id
        )
...
class TestGetFollow:
    ...
    async def test_returns_the_matching_row(self, db_session: AsyncSession) -> None:
        ...
        found = await repo.get_follow(user.id, topic.id)
        assert found is not None
        assert found.id == follower.id


class TestListFollowersAndFollowing:
    async def test_list_followers_scopes_to_the_topic(self, db_session: AsyncSession) -> None:
        organization, user_a = await persist_org_user(db_session)
        _, user_b = await persist_org_user(db_session)
        topic_a = await persist_topic(db_session)
        topic_b = await persist_topic(db_session)
        repo = SqlAlchemyTopicFollowerRepository(db_session)

        follow_a = TopicFollower.create(
            user_id=user_a.id, organization_id=organization.id, topic_id=topic_a.id
        )
        follow_b = TopicFollower.create(
            user_id=user_b.id, organization_id=organization.id, topic_id=topic_b.id
        )
        await repo.add(follow_a)
        await repo.add(follow_b)
        await db_session.commit()

        results, _ = await repo.list_followers(topic_a.id)
    ...
    async def test_list_following_scopes_to_the_user(self, db_session: AsyncSession) -> None:
        ...
        follow_a = TopicFollower.create(
            user_id=user.id, organization_id=organization.id, topic_id=topic_a.id
        )
        await repo.add(follow_a)
        await db_session.commit()
        follow_b = TopicFollower.create(
            user_id=user.id, organization_id=organization.id, topic_id=topic_b.id
        )
        await repo.add(follow_b)
        await db_session.commit()
    ...
    async def test_respects_cursor_pagination(self, db_session: AsyncSession) -> None:
        ...
        for _ in range(3):
            ...
            follow = TopicFollower.create(
                user_id=follower_user.id, organization_id=organization.id, topic_id=topic.id
            )
            await repo.add(follow)
            await db_session.commit()
            created.append(follow.id)

        first_page, next_cursor = await repo.list_followers(topic.id, limit=2)
        assert len(first_page) == 2
        assert next_cursor is not None

        second_page, second_cursor = await repo.list_followers(
            topic.id, cursor=next_cursor, limit=2
        )
...
class TestCountFollowers:
    async def test_counts_followers_of_the_topic(self, db_session: AsyncSession) -> None:
        ...
        assert await repo.count_followers(topic.id) == 3
    ...
    async def test_returns_zero_for_a_topic_with_no_followers(
        ...
        topic = await persist_topic(db_session)
        repo = SqlAlchemyTopicFollowerRepository(db_session)
        assert await repo.count_followers(topic.id) == 0
...
class TestUniqueUserTopicConstraint:
    async def test_duplicate_follow_row_violates_the_constraint(
        ...
        first = TopicFollower.create(
            user_id=user.id, organization_id=organization.id, topic_id=topic.id
        )
        await repo.add(first)
        await db_session.commit()

        second = TopicFollower.create(
            user_id=user.id, organization_id=organization.id, topic_id=topic.id
        )
