"""Shared setup helpers for Differential Diagnosis module repository
tests — every test needs a real, persisted `organizations` row, a
`patients` row, a `doctors` row, a `patient_visits` row, *and* a
`clinical_notes` row (one level deeper than every visit sub-module's own
helpers, since `differential_diagnoses` extends `clinical_notes` rather
than `patient_visits` directly) to satisfy `differential_diagnoses`'
foreign keys. `persist_clinical_reasoning` additionally supports the
tests exercising `clinical_reasoning_id`. Kept local to this test package
rather than in `app/`, matching the identical
`persist_organization`/`persist_patient`/`persist_doctor`/`persist_visit`/
`persist_clinical_note` sequence
`tests.integration.modules.clinical_reasoning._helpers` already
established.
"""
...
async def persist_organization(db_session: AsyncSession) -> Organization:
    ...
    organization = Organization.create(
        ...
        name="Differential Diagnosis Test Org",
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"differential-diagnosis-test-{_unique_suffix()}@example.com"),
...
async def persist_clinical_reasoning(
    db_session: AsyncSession,
    *,
    organization_id: object,
