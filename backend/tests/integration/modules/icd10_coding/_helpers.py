"""Shared setup helpers for ICD-10 Coding module repository tests —
every test needs a real, persisted `organizations` row, a `patients`
row, a `doctors` row, a `patient_visits` row, *and* a `clinical_notes`
row (one level deeper than every visit sub-module's own helpers, since
`icd10_codes` extends `clinical_notes` rather than `patient_visits`
directly) to satisfy `icd10_codes`' foreign keys. `persist_differential_diagnosis`
additionally supports the tests exercising `differential_diagnosis_id`.
Kept local to this test package rather than in `app/`, matching the
identical `persist_organization`/`persist_patient`/`persist_doctor`/
`persist_visit`/`persist_clinical_note` sequence
`tests.integration.modules.differential_diagnosis._helpers` already
established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"icd10-coding-test-{_unique_suffix()}@example.com"),
...
async def persist_differential_diagnosis(
    db_session: AsyncSession,
    *,
    organization_id: object,
...
async def persist_full_chain(
    db_session: AsyncSession,
) -> tuple[Organization, Patient, Doctor, PatientVisit, ClinicalNote]:
    ...
    return organization, patient, doctor, visit, clinical_note
