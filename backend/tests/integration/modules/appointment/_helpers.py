"""Shared setup helpers for Appointment module repository tests — every
test needs a real, persisted `organizations` row, a `patients` row, and
a `doctors` row to satisfy `appointments`' required foreign keys;
`persist_user` and `persist_visit` additionally support the tests
exercising `booked_by_user_id`/`visit_id`. Kept local to this test
package rather than in `app/`, matching the identical
`persist_organization`/`persist_patient`/`persist_doctor` sequence
...
async def persist_user(db_session: AsyncSession, *, organization_id: object) -> User:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"appointment-test-{_unique_suffix()}@example.com"),
