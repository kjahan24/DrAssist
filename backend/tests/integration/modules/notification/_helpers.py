"""Shared setup helpers for Notification module repository tests — every
test needs a real, persisted `organizations` row and a `users` row to
satisfy `notifications`' required foreign keys. Kept local to this test
...
async def persist_user(db_session: AsyncSession, *, organization_id: object) -> User:
    ...
    user = User.register(
        ...
        email=EmailAddress(f"notification-test-{_unique_suffix()}@example.com"),
