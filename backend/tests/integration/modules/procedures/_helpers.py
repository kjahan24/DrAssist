"""Shared setup helpers for Procedures module repository tests — every
...
procedure would be) to satisfy `visit_procedures`' foreign keys. Kept
...
sequence `tests.integration.modules.diagnosis._helpers` already
established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"procedures-test-{_unique_suffix()}@example.com"),
