"""Shared setup helpers for Clinical Notes module repository tests —
every test needs a real, persisted `organizations` row, a `patients`
row, a `doctors` row, and a `patient_visits` row (all chained the way a
real clinical note would be) to satisfy `clinical_notes`' foreign keys.
...
`persist_visit` sequence `tests.integration.modules.attachments._helpers`
already established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"clinical-notes-test-{_unique_suffix()}@example.com"),
