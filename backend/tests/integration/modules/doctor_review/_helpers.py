"""Shared setup helpers for Doctor Review module repository tests —
...
row to satisfy `doctor_reviews`' foreign keys. Kept local to this test
...
`tests.integration.modules.icd10_coding._helpers` already established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"doctor-review-test-{_unique_suffix()}@example.com"),
