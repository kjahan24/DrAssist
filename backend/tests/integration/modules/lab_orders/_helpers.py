"""Shared setup helpers for Lab Orders module repository tests — every
...
`lab_orders` extends `clinical_notes` rather than `patient_visits`
directly) to satisfy `lab_orders`' foreign keys. Kept local to this test
...
`tests.integration.modules.prescriptions._helpers` already established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"lab-orders-test-{_unique_suffix()}@example.com"),
