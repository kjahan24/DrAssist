"""Shared setup helpers for Community Posts module repository tests —
`community_posts.community_id`/`.organization_id`/`.author_id` all
require real, persisted `communities`/`organizations`/`users` rows (and
`community_post_topics.topic_id`/`community_post_attachments.document_id`
require real `medical_topics`/`medical_documents` rows) to satisfy their
FKs. Kept local to this test package rather than in `app/`, matching the
identical `persist_organization`/`persist_user`/... sequence
...
async def persist_user(db_session: AsyncSession, *, organization_id: object) -> User:
    ...
    user = User.register(
        ...
        email=EmailAddress(f"community-posts-test-{unique_suffix()}@example.com"),
...
async def persist_document(
    ...
    document = MedicalDocument.create(
        ...
        storage_path=f"community-posts-test/{uuid4().hex}.pdf",
