"""Shared setup helpers for Audit Log module repository tests — every
test needs a real, persisted `organizations` row to satisfy `audit_logs`'
required `organization_id` foreign key; `persist_user` additionally
supports tests exercising `actor_user_id`. Kept local to this test
...
async def persist_user(db_session: AsyncSession, *, organization_id: object) -> User:
    ...
    user = User.register(
        ...
        email=EmailAddress(f"audit-log-test-{_unique_suffix()}@example.com"),
