"""Shared setup helpers for Chief Complaints module repository tests —
every test needs a real, persisted `organizations` row, a `patients` row,
a `doctors` row, and a `patient_visits` row (all chained the way a real
chief complaint would be) to satisfy `visit_chief_complaints`' foreign
...
`persist_visit` sequence `tests.integration.modules.vital_signs._helpers`
already established.
"""
...
async def persist_organization(db_session: AsyncSession) -> Organization:
    ...
    organization = Organization.create(
        ...
        name="Chief Complaints Test Org",
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"chief-complaint-test-{_unique_suffix()}@example.com"),
