"""Shared setup helpers for Prescription module repository tests — every
test needs a real, persisted `organizations` row, a `patients` row, a
`doctors` row, a `patient_visits` row, *and* a `clinical_notes` row (one
level deeper than every visit sub-module's own helpers, since
`prescriptions` extends `clinical_notes` rather than `patient_visits`
directly) to satisfy `prescriptions`' foreign keys. Kept local to this
...
`tests.integration.modules.soap_notes._helpers` already established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"prescriptions-test-{_unique_suffix()}@example.com"),
