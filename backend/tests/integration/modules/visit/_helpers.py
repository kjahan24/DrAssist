"""Shared setup helpers for Visit module repository tests — every test
needs a real, persisted `organizations` row, a `patients` row, and a
`doctors` row (the patient and doctor sharing the same organization, the
way a real visit would) to satisfy `patient_visits`' foreign keys. Kept
local to this test package rather than in `app/`, matching the identical
`tests.integration.modules.doctor._helpers.persist_organization` and the
`persist_doctor` sequence `tests.integration.modules.patient._helpers`
already established for the Patient Allergies/Medications/Medical
Conditions tests.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"visit-test-{_unique_suffix()}@example.com"),
