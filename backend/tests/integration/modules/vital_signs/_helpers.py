"""Shared setup helpers for Vital Signs module repository tests — every
...
vital signs record would be) to satisfy `visit_vital_signs`' foreign
...
sequence `tests.integration.modules.visit._helpers` already established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"vital-signs-test-{_unique_suffix()}@example.com"),
