"""Shared fixtures for Community Comments module repository (integration)
tests.
...
uniquely-suffixed organization/user/community/post/question/answer so
tests can run repeatedly against a shared, non-torn-down database without
colliding; there is deliberately no per-test transaction-rollback
wrapper, kept simple to match this project's established verification
pattern — see `tests.integration.modules.community_answers.conftest`'s
