"""Shared setup helpers for Attachments module repository tests — every
...
attachment would be) to satisfy `visit_attachments`' foreign keys. Kept
...
sequence `tests.integration.modules.procedures._helpers` already
established.
"""
...
async def persist_doctor(db_session: AsyncSession, *, organization_id: object) -> Doctor:
    ...
    user = User.register(
        organization_id=organization_id,  # type: ignore[arg-type]
        email=EmailAddress(f"attachments-test-{_unique_suffix()}@example.com"),
