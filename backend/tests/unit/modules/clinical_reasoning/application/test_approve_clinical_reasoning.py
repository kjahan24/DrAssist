"""Unit tests for the `ApproveClinicalReasoning` use case."""

from uuid import uuid4

import pytest

from app.modules.clinical_reasoning.application.dto import ApproveClinicalReasoningInput
...
class TestApproveClinicalReasoning:
    async def test_approves_a_pending_record(
        self,
        ...
        output = await use_case.execute(
            ApproveClinicalReasoningInput(clinical_reasoning_id=reasoning.id)
        )
    ...
    async def test_approving_an_already_approved_record_raises(
        self,
        reasoning_repository: FakeClinicalReasoningRepository,
        unit_of_work: FakeUnitOfWork,
    ) -> None:
    ...
    async def test_unknown_reasoning_record_raises(
        ...
        with pytest.raises(ClinicalReasoningNotFoundError):
            await use_case.execute(ApproveClinicalReasoningInput(clinical_reasoning_id=uuid4()))
