"""Unit tests for `DefaultPatientEducationAnalysisGenerator`, including
...
import app.modules.patient_education_ai.infrastructure.prompts.template_registrar as registrar_module  # noqa: E501
...
def _generator(
    *, ai_gateway: FakeAIGateway | None = None
    ...
    return DefaultPatientEducationAnalysisGenerator(
        ...
        template_selector=DefaultPatientEducationAnalysisTemplateSelector(),
        prompt_builder=DefaultPatientEducationAnalysisPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    async def test_returns_raw_text_and_a_completed_session(self) -> None:
        ...
        raw_text, session = await generator.generate(make_input())

        assert raw_text == '{"result": "ok"}'
        assert session.status is EducationGenerationStatus.COMPLETED
    ...
    async def test_session_carries_provider_and_model_from_the_response(self) -> None:
        ...
        _raw_text, session = await generator.generate(make_input())
    ...
    async def test_session_carries_education_setting_and_language(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            make_input(education_setting=PatientEducationSetting.PEDIATRIC, language="fr")
        )

        assert session.education_setting == "pediatric"
...
class TestStreamGenerate:
    async def test_yields_word_chunks_reconstructing_the_full_text(self) -> None:
        ...
        chunks = [chunk async for chunk in generator.stream_generate(make_input())]
    ...
    async def test_propagates_ai_foundation_errors_unwrapped(self) -> None:
        ...
        with pytest.raises(_FakeFoundationError):
            async for _chunk in generator.stream_generate(make_input()):
                pass
