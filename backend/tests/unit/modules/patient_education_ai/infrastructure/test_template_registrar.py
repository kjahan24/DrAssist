"""Unit tests for `ensure_patient_education_templates_registered` —
...
class TestEnsurePatientEducationTemplatesRegistered:
    async def test_registers_all_eighteen_templates(self) -> None:
        registry = _registry()

        await ensure_patient_education_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(
            *(ensure_patient_education_templates_registered(registry) for _ in range(5))
        )
    ...
    async def test_registered_templates_are_renderable(self) -> None:
        ...
        rendered = await registry.render(
            "patient_education.adult.system", PromptVariables({"language": "en"}), version=1
        )
