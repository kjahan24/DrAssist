"""Unit tests for `ArchiveQuestionService`, using in-memory fakes."""
...
from app.modules.community_questions.application.dto import ArchiveQuestionInput
...
from app.modules.community_questions.domain.events import CommunityQuestionArchived
...
def _seeded() -> (
    ...
    uow = FakeUnitOfWork()
    service = ArchiveQuestionService(
...
class TestArchiveQuestion:
    async def test_author_archives_the_question(self) -> None:
        service, questions, communities, _ = _seeded()
        question = await _seed_question(questions, communities)

        await service.execute(
            ArchiveQuestionInput(question_id=question.id, acting_user_id=question.author_id)
        )

        stored = await questions.get_by_id(question.id)
        assert stored is not None
        assert stored.status is QuestionStatus.ARCHIVED

    async def test_moderator_can_archive_someone_elses_question(self) -> None:
        service, questions, communities, _ = _seeded()
        question = await _seed_question(questions, communities)
        moderator_id = uuid4()
        ...
        await service.execute(
            ArchiveQuestionInput(question_id=question.id, acting_user_id=moderator_id)
        )
        stored = await questions.get_by_id(question.id)
        assert stored is not None
        assert stored.status is QuestionStatus.ARCHIVED

    async def test_plain_member_cannot_archive_someone_elses_question(self) -> None:
        service, questions, communities, _ = _seeded()
        question = await _seed_question(questions, communities)
        member_id = uuid4()
        communities.add_membership(
        ...
        with pytest.raises(InsufficientQuestionRoleError):
            await service.execute(
                ArchiveQuestionInput(question_id=question.id, acting_user_id=member_id)
            )

    async def test_unknown_question_raises(self) -> None:
        service, _, _, _ = _seeded()
        with pytest.raises(QuestionNotFoundError):
            await service.execute(ArchiveQuestionInput(question_id=uuid4(), acting_user_id=uuid4()))
    ...
    async def test_publishes_a_community_question_archived_event(self) -> None:
        service, questions, communities, uow = _seeded()
        question = await _seed_question(questions, communities)
        await service.execute(
            ArchiveQuestionInput(question_id=question.id, acting_user_id=question.author_id)
        )
        assert any(isinstance(e, CommunityQuestionArchived) for e in uow.published_events)
