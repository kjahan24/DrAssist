"""Unit tests for `DefaultDifferentialDiagnosisGenerator`, including its
...
import app.modules.differential_diagnosis_ai.infrastructure.prompts.template_registrar as registrar_module  # noqa: E501
...
from app.modules.differential_diagnosis_ai.domain.enums import ClinicalSetting, GenerationStatus
...
def _generator(*, ai_gateway: FakeAIGateway | None = None) -> DefaultDifferentialDiagnosisGenerator:
    ...
    return DefaultDifferentialDiagnosisGenerator(
        ...
        template_selector=DefaultDifferentialDiagnosisTemplateSelector(),
        prompt_builder=DefaultDifferentialDiagnosisPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    ...
    async def test_session_carries_clinical_setting_and_language_from_the_evidence(
        self,
    ) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            _evidence(clinical_setting=ClinicalSetting.EMERGENCY, language="fr")
        )

        assert session.clinical_setting == "emergency"
