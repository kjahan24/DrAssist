...
production prompt templates (5 `ClinicalSetting`s x system/developer/
user)."""
...
class TestTemplateNaming:
    def test_system_template_name_is_setting_scoped(self) -> None:
        assert (
            system_template_name(ClinicalSetting.OUTPATIENT)
            == "differential_diagnosis_suggestion.outpatient.system"
        )

    def test_developer_template_name_is_setting_scoped(self) -> None:
        assert (
            developer_template_name(ClinicalSetting.EMERGENCY)
            == "differential_diagnosis_suggestion.emergency.developer"
        )

    def test_user_template_name_is_setting_scoped(self) -> None:
        assert (
            user_template_name(ClinicalSetting.GERIATRIC)
            == "differential_diagnosis_suggestion.geriatric.user"
        )

    def test_every_clinical_setting_produces_a_distinct_name_per_slot(self) -> None:
        names = {system_template_name(setting) for setting in ClinicalSetting}
        assert len(names) == len(ClinicalSetting)
...
class TestBuildAllTemplates:
    ...
    def test_user_templates_declare_the_clinical_evidence_variables(self) -> None:
        ...
        for template in user_templates:
            ...
            assert "laboratory_results" in template.variable_names
    ...
    def test_developer_templates_carry_the_json_contract(self) -> None:
        ...
        for template in developer_templates:
            ...
            assert "candidates" in template.template_string
            assert "disease_name" in template.template_string
            assert "urgency_level" in template.template_string
