"""Unit tests for `ensure_differential_diagnosis_templates_registered` —
...
class TestEnsureDifferentialDiagnosisTemplatesRegistered:
    async def test_registers_all_fifteen_templates(self) -> None:
        registry = _registry()

        await ensure_differential_diagnosis_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(
            *(ensure_differential_diagnosis_templates_registered(registry) for _ in range(5))
        )
