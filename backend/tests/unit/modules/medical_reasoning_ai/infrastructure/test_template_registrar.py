"""Unit tests for `ensure_medical_reasoning_templates_registered` —
...
class TestEnsureMedicalReasoningTemplatesRegistered:
    async def test_registers_all_fifteen_templates(self) -> None:
        registry = _registry()

        await ensure_medical_reasoning_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(
            *(ensure_medical_reasoning_templates_registered(registry) for _ in range(5))
        )
    ...
    async def test_registered_templates_are_renderable(self) -> None:
        ...
        rendered = await registry.render(
            "medical_reasoning.outpatient.system", PromptVariables({"language": "en"}), version=1
        )
