"""In-memory test doubles for the Attachments module's repository, Unit
of Work, and the Visit/Doctor modules' cross-module ports
`UploadVisitAttachment` depends on — each implements the exact same
...
class FakeVisitAttachmentRepository(VisitAttachmentRepository):
    def __init__(self) -> None:
        self._attachments: dict[UUID, VisitAttachment] = {}
    ...
    async def get_by_storage_key(self, storage_key: str) -> VisitAttachment | None:
        for attachment in self._attachments.values():
            if attachment.storage_key == storage_key.strip():
                return attachment
        return None
    ...
    async def list_by_visit(self, visit_id: UUID) -> list[VisitAttachment]:
        matches = [a for a in self._attachments.values() if a.visit_id == visit_id]
        return sorted(matches, key=lambda a: a.uploaded_at)
...
class FakeVisitQueryPort(VisitQueryPort):
    ...
    id — `UploadVisitAttachment` calls `get_visit_summary` both to check
...
class FakeDoctorQueryPort(DoctorQueryPort):
    ...
    id — `UploadVisitAttachment` calls `get_doctor_summary` both to check
