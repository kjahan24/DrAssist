"""Unit tests for the `MarkAppointmentNoShow` use case
...
from app.modules.appointment.application.dto import MarkAppointmentNoShowInput
...
class TestMarkAppointmentNoShow:
    async def test_marks_a_scheduled_appointment_as_no_show(
        ...
        output = await use_case.execute(MarkAppointmentNoShowInput(appointment_id=appointment.id))

        assert output.status is AppointmentStatus.NO_SHOW
    ...
    async def test_unknown_appointment_raises(
        ...
        with pytest.raises(AppointmentNotFoundError):
            await use_case.execute(MarkAppointmentNoShowInput(appointment_id=uuid4()))

    async def test_marking_a_checked_in_appointment_as_no_show_raises(
        self, appointment_repository: FakeAppointmentRepository, unit_of_work: FakeUnitOfWork
    ) -> None:
        appointment = _make_appointment()
        ...
        with pytest.raises(InvalidAppointmentStatusTransitionError):
            await use_case.execute(MarkAppointmentNoShowInput(appointment_id=appointment.id))
