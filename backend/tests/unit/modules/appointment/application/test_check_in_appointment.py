"""Unit tests for the `CheckInAppointment` use case
...
from app.modules.appointment.application.dto import CheckInAppointmentInput
...
class TestCheckInAppointment:
    async def test_checks_in_a_scheduled_appointment(
        ...
        output = await use_case.execute(CheckInAppointmentInput(appointment_id=appointment.id))

        assert output.status is AppointmentStatus.CHECKED_IN
    ...
    async def test_unknown_appointment_raises(
        ...
        with pytest.raises(AppointmentNotFoundError):
            await use_case.execute(CheckInAppointmentInput(appointment_id=uuid4()))

    async def test_checking_in_twice_raises(
        self, appointment_repository: FakeAppointmentRepository, unit_of_work: FakeUnitOfWork
    ) -> None:
        appointment = _make_appointment()
        ...
        with pytest.raises(InvalidAppointmentStatusTransitionError):
            await use_case.execute(CheckInAppointmentInput(appointment_id=appointment.id))
