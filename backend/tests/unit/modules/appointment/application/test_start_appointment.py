"""Unit tests for the `StartAppointment` use case (CheckedIn ->
InProgress)."""
...
from app.modules.appointment.application.dto import StartAppointmentInput
...
class TestStartAppointment:
    async def test_starts_a_checked_in_appointment(
        ...
        output = await use_case.execute(StartAppointmentInput(appointment_id=appointment.id))

        assert output.status is AppointmentStatus.IN_PROGRESS
    ...
    async def test_unknown_appointment_raises(
        ...
        with pytest.raises(AppointmentNotFoundError):
            await use_case.execute(StartAppointmentInput(appointment_id=uuid4()))

    async def test_starting_a_scheduled_appointment_raises(
        self, appointment_repository: FakeAppointmentRepository, unit_of_work: FakeUnitOfWork
    ) -> None:
        appointment = _make_appointment()
        await appointment_repository.add(appointment)
        ...
        with pytest.raises(InvalidAppointmentStatusTransitionError):
            await use_case.execute(StartAppointmentInput(appointment_id=appointment.id))
