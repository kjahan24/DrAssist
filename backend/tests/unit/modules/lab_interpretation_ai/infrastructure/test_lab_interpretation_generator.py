"""Unit tests for `DefaultLabInterpretationGenerator`, including its
...
def _generator(*, ai_gateway: FakeAIGateway | None = None) -> DefaultLabInterpretationGenerator:
    ...
    return DefaultLabInterpretationGenerator(
        ...
        template_selector=DefaultLabInterpretationTemplateSelector(),
        prompt_builder=DefaultLabPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    async def test_returns_raw_text_and_a_completed_session(self) -> None:
        ...
        assert session.status is LabInterpretationStatus.COMPLETED
    ...
    async def test_session_carries_lab_setting_and_language_from_the_input(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            _input(lab_setting=LabInterpretationSetting.EMERGENCY, language="fr")
        )

        assert session.lab_setting == "emergency"
