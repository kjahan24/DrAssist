...
class TestTemplateNames:
    def test_are_prefixed_lab_interpretation(self) -> None:
        assert system_template_name(LabInterpretationSetting.OUTPATIENT) == (
            "lab_interpretation.outpatient.system"
        )
        assert developer_template_name(LabInterpretationSetting.EMERGENCY) == (
            "lab_interpretation.emergency.developer"
        )
        assert user_template_name(LabInterpretationSetting.PEDIATRIC) == (
            "lab_interpretation.pediatric.user"
        )
...
class TestBuildAllTemplates:
    ...
    def test_builds_a_triple_for_every_setting(self) -> None:
        ...
        for setting in LabInterpretationSetting:
    ...
    def test_user_template_variables_include_lab_values(self) -> None:
        templates = build_all_templates()
        user_template = next(
            t for t in templates if t.name == user_template_name(LabInterpretationSetting.INPATIENT)
        )
