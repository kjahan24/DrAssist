"""Unit tests for `DefaultRadiologyInterpretationGenerator`, including
...
import app.modules.radiology_interpretation_ai.infrastructure.prompts.template_registrar as registrar_module  # noqa: E501
...
def _generator(
    *, ai_gateway: FakeAIGateway | None = None
    ...
    return DefaultRadiologyInterpretationGenerator(
        ...
        template_selector=DefaultRadiologyInterpretationTemplateSelector(),
        prompt_builder=DefaultRadiologyPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    async def test_returns_raw_text_and_a_completed_session(self) -> None:
        ...
        assert session.status is RadiologyInterpretationStatus.COMPLETED
    ...
    async def test_session_carries_radiology_setting_examination_type_and_language(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            _input(
                radiology_setting=RadiologySetting.EMERGENCY,
                examination_type=RadiologyExaminationType.CT_BRAIN,
                language="fr",
            )
        )

        assert session.radiology_setting == "emergency"
        assert session.examination_type == "ct_brain"
