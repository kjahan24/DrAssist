"""In-memory test doubles for the AI Radiology Interpretation module's
...
def make_finding(**overrides: object) -> RadiologyFinding:
    defaults: dict[str, object] = {
        "description": "No acute cardiopulmonary abnormality",
        "category": RadiologyFindingCategory.NORMAL,
    }
    defaults.update(overrides)
    return RadiologyFinding(**defaults)  # type: ignore[arg-type]


def make_result(**overrides: object) -> RadiologyInterpretationResult:
    defaults: dict[str, object] = {
        "examination_summary": "Chest X-ray reviewed, no acute findings.",
        ...
        "clinical_reasoning": "Grounded in the reported lung fields and cardiac silhouette.",
        "raw_text": '{"examination_summary": "Chest X-ray reviewed, no acute findings."}',
...
class FakeRadiologyInterpreterPort(RadiologyInterpreterPort):
    def __init__(
        self,
        *,
        raw_text: str = '{"examination_summary": "Chest X-ray reviewed, no acute findings."}',
        session: GenerationSession | None = None,
        error: Exception | None = None,
        stream_chunks: list[RadiologyInterpretationStreamChunk] | None = None,
    ) -> None:
        self._raw_text = raw_text
        self._session = session or make_generation_session()
        self._error = error
        self._stream_chunks = stream_chunks or [
            RadiologyInterpretationStreamChunk(delta=raw_text, is_final=True)
        ]
...
class FakeRadiologyInterpretationParserPort(RadiologyInterpretationParserPort):
    def __init__(
        self,
        *,
        result: RadiologyInterpretationResult | None = None,
        error: Exception | None = None,
    ) -> None:
        self._result = result or make_result()
        self._error = error
        self.received: list[tuple[str, RadiologyOutputFormat]] = []
...
class FakeRadiologyInterpretationValidatorPort(RadiologyInterpretationValidatorPort):
    def __init__(self, *, error: Exception | None = None) -> None:
        self._error = error
        self.received: list[RadiologyInterpretationResult] = []
...
class FakeRadiologyInterpretationAuditLoggerPort(RadiologyInterpretationAuditLoggerPort):
...
class FakeFindingExtractionPort(FindingExtractionPort):
    def __init__(
        self,
        *,
        candidates: tuple[RadiologyFinding, ...] = (),
        classification: RadiologyFindingCategory | None = None,
    ) -> None:
...
class FakeMedicalReasoningAIPort(MedicalReasoningAIPort):
    ...
    async def generate_reasoning(
        ...
        raise NotImplementedError("not exercised by radiology_interpretation_ai's own tests")
