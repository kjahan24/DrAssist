"""Unit tests for the `ApproveDifferentialDiagnosis` use case."""
...
class TestApproveDifferentialDiagnosis:
    async def test_approves_a_pending_diagnosis(
        self,
        ...
        output = await use_case.execute(
            ApproveDifferentialDiagnosisInput(differential_diagnosis_id=diagnosis.id)
        )

        assert output.review_status is ReviewStatus.APPROVED
        assert unit_of_work.committed is True

    async def test_approving_an_already_approved_diagnosis_raises(
        self,
        diagnosis_repository: FakeDifferentialDiagnosisRepository,
        unit_of_work: FakeUnitOfWork,
    ) -> None:
    ...
    async def test_unknown_diagnosis_raises(
        ...
        with pytest.raises(DifferentialDiagnosisNotFoundError):
            await use_case.execute(
                ApproveDifferentialDiagnosisInput(differential_diagnosis_id=uuid4())
            )
