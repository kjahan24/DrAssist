"""Unit tests for `infrastructure/prompts/templates.py` — the 12
production prompt templates (4 `CodingSetting`s x system/developer/user).
"""
...
class TestTemplateNaming:
    def test_system_template_name_is_setting_scoped(self) -> None:
        assert (
            system_template_name(CodingSetting.OUTPATIENT) == "icd10_suggestion.outpatient.system"
        )

    def test_developer_template_name_is_setting_scoped(self) -> None:
        assert (
            developer_template_name(CodingSetting.EMERGENCY)
            == "icd10_suggestion.emergency.developer"
        )

    def test_user_template_name_is_setting_scoped(self) -> None:
        assert user_template_name(CodingSetting.FOLLOW_UP) == "icd10_suggestion.follow_up.user"

    def test_every_coding_setting_produces_a_distinct_name_per_slot(self) -> None:
        names = {system_template_name(setting) for setting in CodingSetting}
        assert len(names) == len(CodingSetting)


class TestBuildAllTemplates:
    def test_builds_twelve_templates(self) -> None:
    ...
    def test_user_templates_declare_the_clinical_context_variables(self) -> None:
        ...
        for template in user_templates:
            ...
            assert "existing_diagnoses" in template.variable_names
    ...
    def test_developer_templates_carry_the_json_contract(self) -> None:
        ...
        for template in developer_templates:
            ...
            assert "suggestions" in template.template_string
            assert "icd10_code" in template.template_string
            assert "confidence_score" in template.template_string
