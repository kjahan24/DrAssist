"""Unit tests for `DefaultDrugSafetyAnalysisGenerator`, including its
`stream_generate` (chunked-single-call) streaming support — this task's
own "Reuse AI Foundation streaming" requirement."""
...
def _generator(*, ai_gateway: FakeAIGateway | None = None) -> DefaultDrugSafetyAnalysisGenerator:
    ...
    return DefaultDrugSafetyAnalysisGenerator(
        ...
        template_selector=DefaultDrugSafetyAnalysisTemplateSelector(),
        prompt_builder=DefaultDrugSafetyAnalysisPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    async def test_returns_raw_text_and_a_completed_session(self) -> None:
        ...
        assert session.status is SafetyAnalysisStatus.COMPLETED
    ...
    async def test_session_carries_medication_setting_and_language(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            _input(medication_setting=DrugInteractionSetting.ICU, language="fr")
        )

        assert session.medication_setting == "icu"
