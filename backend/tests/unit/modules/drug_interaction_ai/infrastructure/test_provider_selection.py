...
`app.modules.pathology_interpretation_ai`'s own identical test for why:
constructing the real `Settings` is unnecessary, and mutating the
process-wide `get_settings()` singleton to test each provider branch
