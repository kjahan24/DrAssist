"""Unit tests for `ensure_clinical_note_templates_registered` — including
...
class TestEnsureClinicalNoteTemplatesRegistered:
    async def test_registers_all_fifteen_templates(self) -> None:
        registry = _registry()

        await ensure_clinical_note_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(
            *(ensure_clinical_note_templates_registered(registry) for _ in range(5))
        )
    ...
    async def test_registered_templates_are_renderable(self) -> None:
        ...
        rendered = await registry.render(
            "clinical_note.concise.system", PromptVariables({"language": "en"}), version=1
        )
