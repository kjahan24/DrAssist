...
production prompt templates (5 `NoteStyle`s x system/developer/user)."""
...
class TestTemplateNaming:
    def test_system_template_name_is_style_scoped(self) -> None:
        assert system_template_name(NoteStyle.CONCISE) == "clinical_note.concise.system"

    def test_developer_template_name_is_style_scoped(self) -> None:
        assert developer_template_name(NoteStyle.EMERGENCY) == "clinical_note.emergency.developer"

    def test_user_template_name_is_style_scoped(self) -> None:
        assert user_template_name(NoteStyle.FOLLOW_UP) == "clinical_note.follow_up.user"

    def test_every_note_style_produces_a_distinct_name_per_slot(self) -> None:
        names = {system_template_name(style) for style in NoteStyle}
        assert len(names) == len(NoteStyle)
...
class TestBuildAllTemplates:
    ...
    def test_developer_templates_carry_the_json_contract(self) -> None:
        ...
        for template in developer_templates:
            ...
            assert "chief_complaint" in template.template_string
