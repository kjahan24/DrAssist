"""Unit tests for `infrastructure/prompts/templates.py` — the 18
production prompt templates (6 `PrescribingSetting`s x system/developer/
user)."""
...
class TestTemplateNaming:
    def test_system_template_name_is_setting_scoped(self) -> None:
        assert (
            system_template_name(PrescribingSetting.OUTPATIENT)
            == "prescription_suggestion.outpatient.system"
        )

    def test_developer_template_name_is_setting_scoped(self) -> None:
        assert (
            developer_template_name(PrescribingSetting.EMERGENCY)
            == "prescription_suggestion.emergency.developer"
        )

    def test_user_template_name_is_setting_scoped(self) -> None:
        assert (
            user_template_name(PrescribingSetting.FOLLOW_UP)
            == "prescription_suggestion.follow_up.user"
        )

    def test_every_prescribing_setting_produces_a_distinct_name_per_slot(self) -> None:
        names = {system_template_name(setting) for setting in PrescribingSetting}
        assert len(names) == len(PrescribingSetting)
...
class TestBuildAllTemplates:
    ...
    def test_every_setting_has_a_system_developer_and_user_template(self) -> None:
        ...
        for setting in PrescribingSetting:
    ...
    def test_user_templates_declare_the_clinical_context_variables(self) -> None:
        templates = build_all_templates()
        user_templates = [t for t in templates if t.name.endswith(".user")]
        for template in user_templates:
            assert "chief_complaint" in template.variable_names
            assert "existing_medications" in template.variable_names
            assert "allergies" in template.variable_names
            assert "patient_age" in template.variable_names
            assert "weight_kg" in template.variable_names
            assert "pregnancy_status" in template.variable_names
    ...
    def test_developer_templates_carry_the_json_contract(self) -> None:
        ...
        for template in developer_templates:
            ...
            assert "medications" in template.template_string
            assert "safety_findings" in template.template_string
            assert "generic_name" in template.template_string
