"""Unit tests for `ensure_prescription_templates_registered` — including
the idempotency guarantee (repeated calls, and concurrent calls, must
...
class TestEnsurePrescriptionTemplatesRegistered:
    async def test_registers_all_eighteen_templates(self) -> None:
        registry = _registry()

        await ensure_prescription_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(
            *(ensure_prescription_templates_registered(registry) for _ in range(5))
        )
