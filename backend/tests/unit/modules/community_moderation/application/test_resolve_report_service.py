"""Unit tests for `ResolveReportService`, using in-memory fakes."""
...
from app.modules.community_moderation.application.dto import ResolveReportInput
...
from app.modules.community_moderation.domain.events import ReportResolved
...
def _seeded() -> (
    tuple[
        ResolveReportService, FakeCommunityReportRepository, FakeCommunityQueryPort, FakeUnitOfWork
    ]
):
    reports = FakeCommunityReportRepository()
    communities = FakeCommunityQueryPort()
    uow = FakeUnitOfWork()
    service = ResolveReportService(
...
class TestResolveReport:
    async def test_moves_report_to_resolved(self) -> None:
        ...
        assert output.status is ReportStatus.RESOLVED
        assert output.resolution == "Content removed."

    async def test_resolvable_directly_from_open(self) -> None:
        service, reports, communities, _ = _seeded()
        community_id, moderator_id = uuid4(), uuid4()
        report = _make_report(community_id=community_id)
        await reports.add(report)
        communities.add_membership(
            make_member_summary(
                community_id=community_id, user_id=moderator_id, role=CommunityRole.ADMIN
            )
        )
        output = await service.execute(
            ResolveReportInput(
    ...
    async def test_raises_once_already_resolved(self) -> None:
        service, reports, communities, _ = _seeded()
        community_id, moderator_id = uuid4(), uuid4()
        report = _make_report(community_id=community_id)
        report.resolve(moderator_id=uuid4(), resolution="First resolution.")
        await reports.add(report)
        communities.add_membership(
            make_member_summary(
                community_id=community_id, user_id=moderator_id, role=CommunityRole.MODERATOR
    ...
    async def test_publishes_a_report_resolved_event(self) -> None:
        service, reports, communities, uow = _seeded()
        community_id, moderator_id = uuid4(), uuid4()
        report = _make_report(community_id=community_id)
        await reports.add(report)
        ...
        assert any(isinstance(e, ReportResolved) for e in uow.published_events)
