"""Unit test for the Community Moderation module's health route. Calls
...
from app.modules.community_moderation.presentation.router import (
    get_community_moderation_health,
)


class TestCommunityModerationHealthRoute:
    async def test_returns_ok_status(self) -> None:
        result = await get_community_moderation_health()
        assert result == {"status": "ok", "module": "community_moderation"}
