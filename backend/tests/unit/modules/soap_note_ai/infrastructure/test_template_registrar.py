"""Unit tests for `ensure_soap_note_templates_registered` — including the
...
class TestEnsureSOAPNoteTemplatesRegistered:
    async def test_registers_all_fifteen_templates(self) -> None:
        registry = _registry()

        await ensure_soap_note_templates_registered(registry)
    ...
    async def test_concurrent_calls_register_exactly_once(self) -> None:
        ...
        await asyncio.gather(*(ensure_soap_note_templates_registered(registry) for _ in range(5)))
    ...
    async def test_registered_templates_are_renderable(self) -> None:
        ...
        rendered = await registry.render(
            "soap_note.concise.system", PromptVariables({"language": "en"}), version=1
        )
