"""Unit tests for `DefaultSOAPGenerator`, including its `stream_generate`
...
import app.modules.soap_note_ai.infrastructure.prompts.template_registrar as registrar_module
...
from app.modules.soap_note_ai.domain.enums import GenerationStatus, SOAPStyle
...
from tests.unit.modules.soap_note_ai.application.fakes import FakeAIGateway, FakeCostEstimator
...
def _generator(*, ai_gateway: FakeAIGateway | None = None) -> DefaultSOAPGenerator:
    ...
    return DefaultSOAPGenerator(
        ...
        template_selector=DefaultSOAPTemplateSelector(),
        prompt_builder=DefaultSOAPPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    ...
    async def test_session_carries_soap_style_and_language_from_the_encounter(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            _encounter(soap_style=SOAPStyle.EMERGENCY, language="fr")
        )

        assert session.soap_style == "emergency"
