"""Unit tests for the `MarkNotificationRead` use case."""

from uuid import uuid4

import pytest

from app.modules.notification.application.dto import MarkNotificationReadInput
...
class TestMarkNotificationRead:
    async def test_marks_a_delivered_notification_read(
        ...
        output = await use_case.execute(MarkNotificationReadInput(notification_id=notification.id))

        assert output.status is NotificationStatus.READ
    ...
    async def test_unknown_notification_raises(
        ...
        with pytest.raises(NotificationNotFoundError):
            await use_case.execute(MarkNotificationReadInput(notification_id=uuid4()))

    async def test_sent_notification_cannot_be_marked_read(
        self, notification_repository: FakeNotificationRepository, unit_of_work: FakeUnitOfWork
    ) -> None:
        notification = _make_notification()
        notification.mark_sent()
        ...
        with pytest.raises(InvalidNotificationStatusTransitionError):
            await use_case.execute(MarkNotificationReadInput(notification_id=notification.id))
