"""Unit tests for the `ExpireNotification` use case."""

from uuid import uuid4

import pytest

from app.modules.notification.application.dto import ExpireNotificationInput
...
class TestExpireNotification:
    async def test_expires_a_pending_notification(
        ...
        output = await use_case.execute(ExpireNotificationInput(notification_id=notification.id))

        assert output.status is NotificationStatus.EXPIRED
    ...
    async def test_unknown_notification_raises(
        ...
        with pytest.raises(NotificationNotFoundError):
            await use_case.execute(ExpireNotificationInput(notification_id=uuid4()))

    async def test_sent_notification_cannot_be_expired(
        self, notification_repository: FakeNotificationRepository, unit_of_work: FakeUnitOfWork
    ) -> None:
        notification = _make_notification()
        ...
        with pytest.raises(InvalidNotificationStatusTransitionError):
            await use_case.execute(ExpireNotificationInput(notification_id=notification.id))
