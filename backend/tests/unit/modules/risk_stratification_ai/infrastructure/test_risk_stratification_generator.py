"""Unit tests for `DefaultRiskStratificationAnalysisGenerator`, including
...
import app.modules.risk_stratification_ai.infrastructure.prompts.template_registrar as registrar_module  # noqa: E501
...
def _generator(
    *, ai_gateway: FakeAIGateway | None = None
    ...
    return DefaultRiskStratificationAnalysisGenerator(
        ...
        template_selector=DefaultRiskStratificationAnalysisTemplateSelector(),
        prompt_builder=DefaultRiskStratificationAnalysisPromptBuilder(ai_gateway=gateway),
...
class TestGenerate:
    async def test_returns_raw_text_and_a_completed_session(self) -> None:
        ...
        assert session.status is RiskAnalysisStatus.COMPLETED
    ...
    async def test_session_carries_risk_setting_and_language(self) -> None:
        generator = _generator()

        _raw_text, session = await generator.generate(
            make_input(risk_setting=RiskStratificationSetting.ICU, language="fr")
        )

        assert session.risk_setting == "icu"
