"""Unit tests for `RestoreCommentService`, using in-memory fakes."""

from uuid import uuid4

import pytest

from app.modules.community_comments.application.dto import RestoreCommentInput
...
class TestRestoreComment:
    async def test_restores_an_archived_comment_to_draft(self) -> None:
        service, comments, communities, _ = _seeded()
        author_id, community_id = uuid4(), uuid4()
        comment = await _seed_archived_comment(
            comments, author_id=author_id, community_id=community_id
        )
        ...
        output = await service.execute(
            RestoreCommentInput(comment_id=comment.id, acting_user_id=author_id)
        )
        assert output.status is CommentStatus.DRAFT

    async def test_restoring_a_published_comment_raises(self) -> None:
        service, comments, communities, _ = _seeded()
        author_id, community_id = uuid4(), uuid4()
        comment = CommunityComment.create(
            target_type=CommentTargetType.POST,
        ...
        with pytest.raises(CommentCannotBeRestoredError):
            await service.execute(
                RestoreCommentInput(comment_id=comment.id, acting_user_id=author_id)
            )

    async def test_plain_member_cannot_restore_another_authors_comment(self) -> None:
        service, comments, communities, _ = _seeded()
        ...
        with pytest.raises(InsufficientCommentRoleError):
            await service.execute(
                RestoreCommentInput(comment_id=comment.id, acting_user_id=other_id)
            )

    async def test_unknown_comment_raises(self) -> None:
        service, _, _, _ = _seeded()
        with pytest.raises(CommentNotFoundError):
            await service.execute(RestoreCommentInput(comment_id=uuid4(), acting_user_id=uuid4()))
    ...
    async def test_commits_the_unit_of_work(self) -> None:
        ...
        await service.execute(RestoreCommentInput(comment_id=comment.id, acting_user_id=author_id))
