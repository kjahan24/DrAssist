"""Unit tests for `ArchiveCommentService`, using in-memory fakes."""

from uuid import uuid4

import pytest

from app.modules.community_comments.application.dto import ArchiveCommentInput
...
class TestArchiveComment:
    async def test_sets_status_to_archived(self) -> None:
        ...
        output = await service.execute(
            ArchiveCommentInput(comment_id=comment.id, acting_user_id=author_id)
        )
        assert output.status is CommentStatus.ARCHIVED
    ...
    async def test_already_archived_raises(self) -> None:
        ...
        await service.execute(ArchiveCommentInput(comment_id=comment.id, acting_user_id=author_id))

        with pytest.raises(CommentAlreadyArchivedError):
            await service.execute(
                ArchiveCommentInput(comment_id=comment.id, acting_user_id=author_id)
            )

    async def test_plain_member_cannot_archive_another_authors_comment(self) -> None:
        service, comments, communities, _ = _seeded()
        ...
        with pytest.raises(InsufficientCommentRoleError):
            await service.execute(
                ArchiveCommentInput(comment_id=comment.id, acting_user_id=other_id)
            )

    async def test_unknown_comment_raises(self) -> None:
        service, _, _, _ = _seeded()
        with pytest.raises(CommentNotFoundError):
            await service.execute(ArchiveCommentInput(comment_id=uuid4(), acting_user_id=uuid4()))
