"""Unit tests for `DeletePostService`, using in-memory fakes."""
...
def _seeded() -> (
    tuple[DeletePostService, FakeCommunityPostRepository, FakeCommunityQueryPort, FakeUnitOfWork]
):
...
class TestDeletePost:
    async def test_author_removes_the_post(self) -> None:
        service, posts, communities, _ = _seeded()
        post = await _seed_post(posts, communities)

        await service.execute(DeletePostInput(post_id=post.id, acting_user_id=post.author_id))

        assert await posts.get_by_id(post.id) is None

    async def test_moderator_can_delete_someone_elses_post(self) -> None:
        service, posts, communities, _ = _seeded()
        post = await _seed_post(posts, communities)
        moderator_id = uuid4()
        ...
        await service.execute(DeletePostInput(post_id=post.id, acting_user_id=moderator_id))

        assert await posts.get_by_id(post.id) is None

    async def test_plain_member_cannot_delete_someone_elses_post(self) -> None:
        service, posts, communities, _ = _seeded()
        post = await _seed_post(posts, communities)
        member_id = uuid4()
        communities.add_membership(
        ...
        with pytest.raises(InsufficientPostRoleError):
            await service.execute(DeletePostInput(post_id=post.id, acting_user_id=member_id))

    async def test_unknown_post_raises(self) -> None:
        service, _, _, _ = _seeded()
        with pytest.raises(PostNotFoundError):
            await service.execute(DeletePostInput(post_id=uuid4(), acting_user_id=uuid4()))
