...
Needed because several peer modules' use cases resolve a `doctor_id`
through `DoctorQueryPort` and raise `DoctorNotFoundError` when it is
missing — before this file existed, the only way to do that was to
import directly from `app.modules.doctor.domain.exceptions`, which
...
the exception type itself, so every existing `except DoctorNotFoundError`
