"""`DeleteQuestionService` — the author (or a community moderator/admin)
may delete their own question, the same authorization rule
`UpdateQuestionService` uses.

Unlike `app.modules.community_posts.application.services
.delete_post_service.DeletePostService` (which calls
`CommunityPostRepository.remove`, a repository-level soft delete),
deletion here is a domain-level status transition
(`CommunityQuestion.delete()`), persisted through the ordinary `add()`
upsert — see `QuestionStatus`'s own docstring for why."""
...
class DeleteQuestionService:
    ...
    async def execute(self, input_dto: DeleteQuestionInput) -> None:
