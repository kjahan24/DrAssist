"""`ReopenQuestionService` — author-or-moderator authorized, the same
rule `UpdateQuestionService` uses. `CommunityQuestion.reopen()` itself
enforces the CLOSED-only precondition — see that method's own docstring.
"""
...
class ReopenQuestionService:
    ...
    async def execute(self, input_dto: ReopenQuestionInput) -> CommunityQuestionSummaryDTO:
