"""`PublishPostService` — moves a `CommunityPost` from `DRAFT` (or
`ARCHIVED`) to `PUBLISHED`, stamping `published_at`. Author-or-moderator
authorized, the same rule `UpdatePostService` uses."""

from app.modules.community.public.interfaces import CommunityQueryPort
from app.modules.community_posts.application.dto import CommunityPostSummaryDTO, PublishPostInput
...
class PublishPostService:
    ...
    async def execute(self, input_dto: PublishPostInput) -> CommunityPostSummaryDTO:
