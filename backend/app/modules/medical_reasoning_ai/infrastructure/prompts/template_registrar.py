"""`ensure_medical_reasoning_templates_registered` — idempotently
...
`app.modules.differential_diagnosis_ai.infrastructure.prompts
.template_registrar.ensure_differential_diagnosis_templates_registered`
establishes for its own module (see that module's own docstring for the
full reasoning: why this happens lazily rather than via an `app/main.py`
startup hook, and why the lock+flag guard exists).
...
from app.modules.medical_reasoning_ai.infrastructure.prompts.templates import build_all_templates

_registration_lock = asyncio.Lock()
_templates_registered = False


async def ensure_medical_reasoning_templates_registered(registry: PromptRegistry) -> None:
