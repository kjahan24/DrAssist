"""`DefaultLabInterpretationGenerator` — the one concrete
`LabInterpreterPort` implementation this task ships: selects a prompt
...
Streaming does **not** go through `InterpretLabResultsUseCase`'s
...
from app.modules.lab_interpretation_ai.domain.enums import LabInterpretationStatus
...
class DefaultLabInterpretationGenerator(LabInterpreterPort):
    def __init__(
        self,
        *,
        ai_gateway: AIGatewayPort,
        prompt_registry: PromptRegistry,
        template_selector: LabInterpretationTemplateSelectorPort,
        prompt_builder: LabPromptBuilderPort,
    ...
    async def generate(self, input_dto: LabInterpretationInput) -> tuple[str, GenerationSession]:
        await ensure_lab_interpretation_templates_registered(self._prompt_registry)
        template_set = self._template_selector.select(input_dto.lab_setting)
        ...
        session = GenerationSession(
            ...
            provider=response.provider.value,
            model=response.model.name,
            lab_setting=input_dto.lab_setting.value,
    ...
    async def stream_generate(
        ...
        for delta, is_final in chunk_text_by_word(raw_text):
            yield LabInterpretationStreamChunk(delta=delta, is_final=is_final)
