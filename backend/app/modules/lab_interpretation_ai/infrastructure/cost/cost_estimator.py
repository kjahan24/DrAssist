...
`app.modules.medical_reasoning_ai.infrastructure.cost.cost_estimator
...
it cannot be imported from here (module-independence rule); the "each
module defines its own copy" precedent
(`app.modules.documents`/`app.modules.attachments`'s own
`Sha256Checksum`, and now seven AI modules' own `CostEstimator` copies)
is the more consistent choice than inventing a new shared abstraction for
an eighth module's identical need.
"""
