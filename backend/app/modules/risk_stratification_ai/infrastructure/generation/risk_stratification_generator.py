"""`DefaultRiskStratificationAnalysisGenerator` — the one concrete
`RiskStratificationAnalysisGeneratorPort` implementation this task
ships: selects a prompt template set, renders it, and calls AI
Foundation's public `AIGatewayPort.generate_chat_completion` — never a
...
Streaming does **not** go through `AnalyzePatientRiskUseCase`'s
...
from app.modules.risk_stratification_ai.domain.enums import RiskAnalysisStatus
from app.modules.risk_stratification_ai.domain.value_objects import (
    GenerationSession,
    RiskStratificationInput,
    RiskStratificationStreamChunk,
)
from app.modules.risk_stratification_ai.infrastructure.prompts.template_registrar import (
    ensure_risk_stratification_templates_registered,
)
from app.shared.infrastructure.text_processing.word_chunking import chunk_text_by_word


class DefaultRiskStratificationAnalysisGenerator(RiskStratificationAnalysisGeneratorPort):
    def __init__(
        self,
        *,
        ai_gateway: AIGatewayPort,
        prompt_registry: PromptRegistry,
        template_selector: RiskStratificationAnalysisTemplateSelectorPort,
        prompt_builder: RiskStratificationAnalysisPromptBuilderPort,
    ...
    async def generate(self, input_dto: RiskStratificationInput) -> tuple[str, GenerationSession]:
        await ensure_risk_stratification_templates_registered(self._prompt_registry)
        template_set = self._template_selector.select(input_dto.risk_setting)
        ...
        session = GenerationSession(
            ...
            provider=response.provider.value,
            model=response.model.name,
            risk_setting=input_dto.risk_setting.value,
    ...
    async def stream_generate(
        ...
        for delta, is_final in chunk_text_by_word(raw_text):
            yield RiskStratificationStreamChunk(delta=delta, is_final=is_final)
