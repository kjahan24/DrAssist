"""`RestoreCommentService` — author-or-moderator authorized, the same
rule `UpdateCommentService` uses. Used uniformly for both top-level
comments and replies. Always lands on `DRAFT` — see
...
class RestoreCommentService:
    ...
    async def execute(self, input_dto: RestoreCommentInput) -> CommunityCommentSummaryDTO:
