...
`RecordVisitProcedure` needs both the Visit and Doctor modules' public
...
`app.modules.diagnosis.api.dependencies` established.
"""
...
def get_procedure_repository(session: DbSession) -> VisitProcedureRepository:
    return SqlAlchemyVisitProcedureRepository(session)
...
ProcedureRepo = Annotated[VisitProcedureRepository, Depends(get_procedure_repository)]
...
def get_procedure_query_service(
    procedure_repository: ProcedureRepo,
) -> VisitProcedureQueryService:
    return VisitProcedureQueryService(procedure_repository=procedure_repository)


def get_record_procedure_use_case(
    procedure_repository: ProcedureRepo,
