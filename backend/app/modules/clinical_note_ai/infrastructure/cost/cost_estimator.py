...
`app.modules.ai_copilot.infrastructure.cost.cost_estimator.CostEstimator`
...
imported from here; see this module's own `application/ports.py
::CostEstimatorPort` for the structural-typing seam this satisfies.
`ChatCompletionResponse` (AI Foundation's public response shape) carries
token usage but no cost figure — cost estimation happens inside that
module's own `infrastructure/providers/resilient_provider.py` decorator,
