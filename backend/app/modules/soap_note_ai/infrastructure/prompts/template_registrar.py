"""`ensure_soap_note_templates_registered` — idempotently registers this
module's 15 prompt templates (`templates.py`) into AI Foundation's
shared, process-lifetime `PromptRegistry`
(`app.modules.ai.container.get_prompt_registry`), the same pattern
`app.modules.clinical_note_ai.infrastructure.prompts.template_registrar
.ensure_clinical_note_templates_registered` establishes for its own
...
from app.modules.soap_note_ai.infrastructure.prompts.templates import build_all_templates

_registration_lock = asyncio.Lock()
_templates_registered = False


async def ensure_soap_note_templates_registered(registry: PromptRegistry) -> None:
