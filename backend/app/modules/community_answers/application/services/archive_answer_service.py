"""`ArchiveAnswerService` — author-or-moderator authorized, the same
rule `UpdateAnswerService` uses. `CommunityAnswer.archive()` itself
safely clears a stale best-answer flag — see that method's own
docstring."""
...
class ArchiveAnswerService:
    ...
    async def execute(self, input_dto: ArchiveAnswerInput) -> CommunityAnswerSummaryDTO:
