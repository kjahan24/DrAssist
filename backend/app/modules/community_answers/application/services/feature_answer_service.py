"""`FeatureAnswerService` — community-moderator-only, no author
exception — see `CommunityAnswer.set_featured`'s own docstring."""
...
class FeatureAnswerService:
    ...
    async def execute(self, input_dto: SetAnswerFeaturedInput) -> None:
        ...
        ensure_is_moderator(
            member, community_id=answer.community_id, user_id=input_dto.acting_user_id
        )

        answer.set_featured(input_dto.featured)
