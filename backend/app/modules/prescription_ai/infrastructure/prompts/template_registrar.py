"""`ensure_prescription_templates_registered` — idempotently registers
this module's 18 prompt templates (`templates.py`) into AI Foundation's
...
.ensure_icd10_templates_registered` establishes for its own module (see
that module's own docstring for the full reasoning: why this happens
lazily rather than via an `app/main.py` startup hook, and why the
...
from app.modules.prescription_ai.infrastructure.prompts.templates import build_all_templates

_registration_lock = asyncio.Lock()
_templates_registered = False


async def ensure_prescription_templates_registered(registry: PromptRegistry) -> None:
