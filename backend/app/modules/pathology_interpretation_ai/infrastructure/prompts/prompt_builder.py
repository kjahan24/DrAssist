"""`DefaultPathologyPromptBuilder` — the one concrete
`PathologyPromptBuilderPort` implementation this task ships: renders a
`PathologyInterpretationTemplateSet`'s three templates via AI
Foundation's public `AIGatewayPort.render_prompt`, turning
`PathologyInterpretationInput`'s fields into the flat string
...
from app.modules.pathology_interpretation_ai.application.ports import PathologyPromptBuilderPort
...
class DefaultPathologyPromptBuilder(PathologyPromptBuilderPort):
    def __init__(self, *, ai_gateway: AIGatewayPort) -> None:
        self._ai_gateway = ai_gateway

    def build_variables(self, input_dto: PathologyInterpretationInput) -> PromptVariables:
        return PromptVariables(
            {
                ...
                "radiology_interpretation": _text_or_default(input_dto.radiology_interpretation),
