"""`StructlogPathologyInterpretationAuditLogger` — the one concrete
`PathologyInterpretationAuditLoggerPort` implementation this task ships,
...
from app.modules.pathology_interpretation_ai.domain.value_objects import GenerationSession

logger = get_logger(__name__)


class StructlogPathologyInterpretationAuditLogger(PathologyInterpretationAuditLoggerPort):
    async def log_generation(
        self, session: GenerationSession, *, organization_id: UUID, patient_id: UUID
    ) -> None:
        logger.info(
            "pathology_interpretation_generation",
            generation_id=str(session.generation_id),
            organization_id=str(organization_id),
            patient_id=str(patient_id),
            provider=session.provider,
            model=session.model,
            pathology_setting=session.pathology_setting,
    ...
    async def log_failure(
        ...
        *,
        ...
        logger.warning(
            "pathology_interpretation_generation_failed",
