"""Domain events published by Doctor Review module aggregates.
...
class DoctorReviewCreated(DomainEvent):
...
@dataclass(frozen=True, kw_only=True)
class DoctorReviewUpdated(DomainEvent):
    doctor_review_id: UUID
    clinical_note_id: UUID


@dataclass(frozen=True, kw_only=True)
class DoctorReviewStatusChanged(DomainEvent):
    doctor_review_id: UUID
    clinical_note_id: UUID
    review_status: str
